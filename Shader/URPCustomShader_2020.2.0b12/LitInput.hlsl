...
half _Surface;
half _UVSec;
half _OnlyShadows;              // CustomCode!
half4 _OnlyShadowColor;         // CustomCode!
CBUFFER_END
...
UNITY_DOTS_INSTANCING_START(MaterialPropertyMetadata)
    ...
    UNITY_DOTS_INSTANCED_PROP(float , _UVSec)
    UNITY_DOTS_INSTANCED_PROP(float , _OnlyShadows)             // CustomCode!
    UNITY_DOTS_INSTANCED_PROP(float4 , _OnlyShadowColor)        // CustomCode!
...
#define _UVSec                  UNITY_ACCESS_DOTS_INSTANCED_PROP_FROM_MACRO(float  , Metadata__UVSec)
#define _OnlyShadows            UNITY_ACCESS_DOTS_INSTANCED_PROP_FROM_MACRO(float  , Metadata__OnlyShadows)                   // CustomCode!
#define _OnlyShadowColor        UNITY_ACCESS_DOTS_INSTANCED_PROP_FROM_MACRO(float  , Metadata__OnlyShadowColor)               // CustomCode!
#endif
...
// CustomCode!
inline void InitializeStandardLitSurfaceData_Custom(float4 uv, out SurfaceData outSurfaceData)
{
    half4 albedoAlpha = SampleAlbedoAlpha(uv, TEXTURE2D_ARGS(_BaseMap, sampler_BaseMap));
    //half4 albedoAlpha2 = SAMPLE_TEXTURE2D(_OcclusionMap, sampler_OcclusionMap, uv.zw);
    //albedoAlpha = lerp(albedoAlpha, albedoAlpha2, _OcclusionStrength);
    outSurfaceData.alpha = Alpha(albedoAlpha.a, _BaseColor, _Cutoff);
    ...
    outSurfaceData.smoothness = specGloss.a;
    outSurfaceData.normalTS = SampleNormal(uv, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap), _BumpScale);
    outSurfaceData.occlusion = SampleOcclusion(uv.zw);
    outSurfaceData.emission = SampleEmission(uv, _EmissionColor.rgb, TEXTURE2D_ARGS(_EmissionMap, sampler_EmissionMap));
...
#if defined(_DETAIL)
    half detailMask = SAMPLE_TEXTURE2D(_DetailMask, sampler_DetailMask, uv.zw).a;
    float2 detailUv = uv.zw * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
    outSurfaceData.albedo = ApplyDetailAlbedo(detailUv, outSurfaceData.albedo, detailMask);
    outSurfaceData.normalTS = ApplyDetailNormal(detailUv, outSurfaceData.normalTS, detailMask);
